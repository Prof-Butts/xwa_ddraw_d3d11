...
std::vector<std::wstring> ListFiles(std::string path)
{
	...
	log_debug("[DBG] [CUBE] Listing files under [%s]", path.c_str());
	if (!fs::exists(path) || !fs::is_directory(path)) {
		log_debug("[DBG] [CUBE] ERROR: Directory %s does not exist", path.c_str());
		return result;
	}
	...
	for (const auto& entry : fs::directory_iterator(path)) {
		...
		if (fs::is_regular_file(entry.status())) {
			...
			log_debug("[DBG] [CUBE]   File: [%s]-[%d]", fullName.c_str(), key);
			keys.push_back(key);
	...
	float *diffuseMipLevel_out)
{
	...
	*diffuseMipLevel_out = 0;
	...
	{
		if (!g_config.EnableSideProcess || !g_bXwaDDrawPlayerPresent)
		{
			ID3D11ShaderResourceView* cubeFaceSRV = nullptr;

			// Using the "context" here and the SRV, causes mip-maps to be auto-generated by WIC.
			// Apparently, we need to do this in order for CopySubResourceRegion to work below.
			HRESULT res = DirectX::CreateWICTextureFromFile(device, context,
				fileNames[cubeFaceIdx].c_str(), (ID3D11Resource**)&cubeFace, &cubeFaceSRV);
		}
		else
		{
			HANDLE handle;
		...
		{
			log_debug("[DBG] [CUBE] COULD NOT LOAD CUBEFACE [%d]. Error: 0x%x", cubeFaceIdx, res);
			return false;
		}
	}
